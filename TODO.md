...
- [x] **Bug 16: MapLibre WebGL Instance Destroyed on Every Stop Click**
...
- [x] **Bug 1: Day 1 Flight Date Filter Hardcoded to Legacy Dates**
...
- [x] **Bug 9: Offline State Desync in Vault**
...
- [x] **Bug 5: No "+ Add Place / Stop" Action in Timeline**
...
- [x] **Bug 4: Read-Only Stop Modal (No Edit or Delete)**
...
- [x] **Bug 6: No Day Creation or Deletion in Workspace**
...
- [x] **Bug 10: White Screen Crash on Undefined Weather**
...
- [x] **Bug 17: Timeline Stop Click Does Not Highlight Marker on Map**
...
- [x] **Bug 19: Crash on Undefined Address in TimelineCard**
...
- [x] **Bug 20: Documents Search Filter Crash on Missing Flight Attributes**
...
- [x] **Bug 2: DaySelector Date String Parsing Fragility**
...
- [x] **Bug 11: Missing Currencies in TripSettingsPage**
...
- [x] **Bug 12: Unsaved Theme Color in TripSettingsPage**
...
- [x] **Bug 3: Hardcoded Dollar Sign in Expense Tracker**
...
- [x] **Bug 7: TSP Route Optimizer Inverts Chronological Stop Times**
...
- [x] **Bug 13: Date Filter Raw String Comparison in TripsListPage**
...
- [x] **Bug 14: Stale Modal State in TripManagerModal**
...
- [x] **Bug 18: `createDefaultTrip` Ignores Date Range**
...
- [x] **Bug 8: GPX Export Default Filename & Metadata Decoupled from Trip**
...
- [x] **Bug 15: Leftover Legacy Branding in ReadinessModal**
...
|---|---|---|---|---|
| **F1** | **Unassigned "Places to Visit" Bucket (Ideas Drawer)** | Unscheduled pool of places/attractions/restaurants that travelers want to visit but haven't yet assigned to a specific day or time. Users can drag or assign them to days later. | **Completed** (PlacesToVisitDrawer, Quick OSM Add, Day Assignment, Unassign to Ideas) | **P0 (Core)** |
| **F2** | **Interactive Place Search & Autocomplete (Geocoding)** | Search bar where typing a place ("Louvre", "Petronas Towers", "Starbucks") queries a geocoder/POI database (OpenStreetMap / Nominatim) to automatically fetch coordinates, address, and place category. | **Completed** (PlaceSearchInput debounced Nominatim OSM geocoding with category inference) | **P0 (Core)** |
| **F3** | **Expense Splitting & Debt Settlement ("Who Owes Whom")** | Splitwise-like group balances: multi-traveler split allocation (split equally or specific amounts), net balances summary ("Alex is owed $45, Taylor owes $45"), and "Settle Up" debt simplification. | **Completed** (Multi-traveler balances, greedy debt minimization, Settle Up payments) | **P0 (Core)** |
| **F4** | **Stop CRUD (Add, Edit, Delete, Reorder)** | Complete modal/form to edit stop title, address, time, duration, notes, category, and delete or reorder stops up/down. | **Completed** (Add Place modal, interactive StopDetailModal with edit/delete/move) | **P0 (Core)** |
| **F5** | **Day CRUD (Add Day, Delete Day, Reorder Days)** | Button to append new days, extend trip date range, or delete days with their scheduled stops. | **Completed** (+ Add Day tab pill with auto-dates, Delete Day with renumbering) | **P0 (Core)** |
| **F6** | **Schedule Overlap & Transit Conflict Detection** | Warning pill between stops if `Stop A endTime + transitDuration > Stop B startTime`, alerting travelers that they will arrive late. | **Completed** (`detectTransitConflict`, `DistancePill` late warning badge) | **P1** |
| **F7** | **Categorized Packing Lists** | Full packing checklist categorized into Clothes, Toiletries, Electronics, Documents, Essentials with progress bars and item management. | **Completed** (5-category packing checklist in `ReadinessModal`, progress metrics, add/check/delete) | **P1** |
| **F8** | **iCalendar (`.ics`) Export / Calendar Sync** | Downloadable `.ics` calendar feed containing all flights, hotel stays, and timed itinerary stops to sync with Google Calendar, Apple Calendar, and Outlook. | **Completed** (`generateIcalendarFeed`, RFC 5545 generator in `ShareModal` & icalExport) | **P1** |
| **F9** | **Trip Scratchpad & General Notes** | Trip-level and day-level free-form notes section for emergency contacts, embassy addresses, packing reminders, and wifi passwords. | **Completed** (`ScratchpadModal`, `TripSettingsPage`, day notes indicator & navbar access) | **P1** |
| **F10**| **Printable Travel Packet View (`@media print`)** | Clean, formatted printer-friendly layout for printing physical emergency travel packets with flight confirmations, vouchers, and daily schedules. | **Completed** (`PrintTravelPacket`, `print.css`, print actions in settings & share modal) | **P2** |
...
- [x] Fix MapLibre WebGL context recreation on stop click (Bug 16)
- [x] Dynamic flight date matching for Day 1 companion flights (Bug 1)
- [x] Synchronous local storage update on vault save (Bug 9)
- [x] Safe weather fallback in WeatherBanner and DaySelector (Bug 10)
- [x] Connect timeline stop click to map marker highlight & flyTo (Bug 17)
- [x] Safe address split in TimelineCard (Bug 19)
- [x] Optional chaining in DocumentsAndTicketsHub search filter (Bug 20)
- [x] Safe date string splitting in DaySelector (Bug 2)
- [x] Dynamic currency formatting in ExpenseTracker (Bug 3)
- [x] Add missing currencies (AED, MYR, etc.) in TripSettingsPage (Bug 11)
- [x] Persist theme color swatch updates to trip days (Bug 12)
- [x] Safe date comparison in TripsListPage (Bug 13)
- [x] Sync activeTrip in TripManagerModal (Bug 14)
- [x] Multi-day date span calculation in `createDefaultTrip` (Bug 18)
- [x] Dynamic GPX metadata naming (Bug 8)
- [x] Remove legacy "TripMojo" branding in ReadinessModal (Bug 15)

### Phase 2: Stop & Day CRUD + Route Timing (Bugs 4–7, Features F4–F5)
- [x] Add "+ Add Place" action in timeline stream (Bug 5, Feature F4)
- [x] Interactive Stop Modal with edit, delete, and "Move to Day" (Bug 4, Feature F4)
- [x] "+ Add Day" and "Delete Day" actions in workspace (Bug 6, Feature F5)
- [x] Chronological start time re-flow after 2-opt TSP optimization (Bug 7)

### Phase 3: Wanderlog Core Capabilities (Features F1–F3) & Route Optimization Preview / Undo
- [x] Unassigned "Places to Visit" Bucket / Ideas Drawer (Feature F1)
- [x] OpenStreetMap / Nominatim place search & geocoding autocomplete (Feature F2)
- [x] Group expense splitting, "Who Owes Whom" balance sheet & "Settle Up" action (Feature F3)
- [x] Route Optimization Preview modal with before/after comparison & replacement prompt
- [x] "Undo Optimization" action in Timeline Header and Interactive Map toolbar

### Phase 4: Travel Utilities Parity (Features F6–F10)
- [x] iCalendar (`.ics`) RFC 5545 export for Google/Apple Calendar (Feature F8)
- [x] Categorized packing list with progress metrics (Feature F7)
- [x] Schedule transit conflict warning alerts (Feature F6)
- [x] Trip scratchpad & general notes section (Feature F9)
- [x] Printable travel packet `@media print` styling (Feature F10)
